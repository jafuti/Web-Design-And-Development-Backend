...
namespace WebApplication14_2021A.Controllers
{
    ...
    {
        private readonly IBrandService _sevice;
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(long id, Brand brand)
        {
            if (!ModelState.IsValid)
            {
                return View(brand);
            ...
            if (id == brand.Id)
            {
                await _sevice.UpdateAsync(id, brand);
                return RedirectToAction(nameof(Index));
            }
            return View(brand);
        }
